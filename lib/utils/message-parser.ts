 ...
 * Groups options by detecting when keys repeat (e.g., second [[A]] starts a new group).
 */
export function parseMultipleChoice(content: string): MultipleChoiceOption[] {
  const groups = parseMultipleChoiceGroups(content)
  // Flatten all groups for backward compat
  return groups.flatMap(g => g.choices)
}

/**
 * Parse multiple-choice options grouped by question.
 * Detects group boundaries when a key repeats (e.g., second [[A]]).
 * Extracts bold text before each group as the header.
 */
export function parseMultipleChoiceGroups(content: string): MultipleChoiceGroup[] {
  const groups: MultipleChoiceGroup[] = []
  let currentGroup: MultipleChoiceGroup = { choices: [] }
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    // If we see a repeated key, start a new group
    if (seenKeys.has(key)) {
      if (currentGroup.choices.length > 0) {
        groups.push(currentGroup)
      }
      // Try to extract a header from text between previous group's last option and this [[A]]
      const header = extractHeaderBefore(content, match.index)
      currentGroup = { header, choices: [] }
      seenKeys.clear()
    }
  ...
  // Try to extract header for the first group
  if (groups.length > 0 && !groups[0].header) {
    const firstChoiceIdx = content.search(/\[\[[A-Z]\]\]/)
    if (firstChoiceIdx > 0) {
      const textBefore = content.substring(0, firstChoiceIdx)
      const headerMatch = textBefore.match(/\*\*([^*]+)\*\*\s*$/m)
      if (headerMatch) {
        groups[0].header = headerMatch[1].trim()
      }
    }
  }
...
/**
 * Extract a bold header from text just before a [[X]] marker position.
 */
function extractHeaderBefore(content: string, position: number): string | undefined {
  // Look backwards from position for a **bold** header
