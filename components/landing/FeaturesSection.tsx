...
const PALETTE_COLORS: Record<ColorPalette, { bg: string; text: string; glow: string }> = {
  violet: { bg: 'bg-violet-500/10', text: 'text-violet-500', glow: 'shadow-violet-500/20' },
  ocean: { bg: 'bg-cyan-500/10', text: 'text-cyan-500', glow: 'shadow-cyan-500/20' },
  sunset: { bg: 'bg-orange-500/10', text: 'text-orange-500', glow: 'shadow-orange-500/20' },
  forest: { bg: 'bg-emerald-500/10', text: 'text-emerald-500', glow: 'shadow-emerald-500/20' },
  midnight: { bg: 'bg-purple-500/10', text: 'text-purple-400', glow: 'shadow-purple-500/20' },
  electric: { bg: 'bg-blue-500/10', text: 'text-blue-500', glow: 'shadow-blue-500/20' },
  rose: { bg: 'bg-rose-500/10', text: 'text-rose-500', glow: 'shadow-rose-500/20' },
  aurora: { bg: 'bg-green-500/10', text: 'text-green-500', glow: 'shadow-green-500/20' },
}
              ...
              return (
                <StaggerItem key={index}>
                  <ScaleOnHover className="h-full">
                    <Card className={`h-full bg-slate-900/50 backdrop-blur-sm border-slate-800 hover:border-slate-600 transition-all duration-300 hover:shadow-xl hover:${colors.glow} group rounded-2xl`}>
              ...
              return (
                <StaggerItem key={index}>
                  <div className="text-center group">
                    <div className={`w-14 h-14 rounded-2xl ${colors.bg} flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300`}>
                ...
                <StaggerItem key={index}>
                  <ScaleOnHover className="h-full">
                    <div className="relative group h-full">
                      <div className={`absolute -inset-0.5 bg-gradient-to-r ${gradient} rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur`} />
                      <Card className="relative h-full bg-white dark:bg-slate-800 border-0 shadow-lg rounded-2xl overflow-hidden">
                        <CardContent className="p-8">
                          ...
                          <div className={`mt-4 flex items-center ${colors.text} font-medium text-sm group-hover:gap-2 transition-all duration-300`}>
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <StaggerItem key={index}>
                <ScaleOnHover className="h-full">
                  <Card className={`h-full border-slate-200 dark:border-slate-800 hover:shadow-xl hover:${colors.glow} transition-all duration-300 rounded-2xl group`}>
                    <CardContent className="p-6 md:p-8">
